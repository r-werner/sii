...
class SolaxX3RS485():
    ...
    def __init__(self):
        # self.port = port
        self.client = ModbusSerialClient(method="rtu", port="/dev/cu.usbserial-14210", baudrate=9600, parity='N', stopbits=1,
                       bytesize=8, timeout=3)
    def fetch(self):
        result = self.client.read_input_registers(0X400, 53, slave = 1)
        #result = self.client.read_input_registers(0X400, 7, slave = 1)
